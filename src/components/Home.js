...
import { FaSearch, FaCalendarPlus, FaStar, FaMapMarkerAlt, FaCity, FaChevronLeft, FaChevronRight, FaTimes, FaEdit, FaSignOutAlt } from 'react-icons/fa';
import API_URL from '../config';

const Home = () => {
  const { user } = useContext(AuthContext);
  const { setShowTripModal } = useTripModal();
  ...
  // Dropdown menu state for trip options
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (openDropdown && !e.target.closest('.trip-options-menu')) {
  ...
  const handleLeaveTrip = async (tripId) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/trips/${tripId}/members/${user.user_id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: user.user_id })
      });
      ...
      if (response.ok) {
        ...
        showToast(data.error || 'Failed to leave trip', 'error');
      }
    } catch (error) {
                  ...
                  >
                    ...
                    {isMobile && (
                      <div className="trip-options-menu">
                        <button 
                          className="trip-options-btn"
                          onClick={(e) => {
                            e.stopPropagation();
                            setOpenDropdown(openDropdown === trip.trip_id ? null : trip.trip_id);
                          }}
                          title="Options"
                        >
                    ...
                    <div className="trip-card-content" onClick={() => navigate(`/planner?trip=${trip.trip_id}`)}>
                      ...
                      <div className="trip-card-actions">
                        {/* Desktop: Show Edit/Delete Buttons */}
                        {!isMobile && isOwner && (
                          <button 
